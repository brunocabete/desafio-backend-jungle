...
| 7 — Test matrix | ✅ passo 0 (schema): `uq_wager_single_reversal` (migration + snapshot + enforcement — §27). ✅ **multi-instância: ≥3 processos reais simultâneos** (`test/multi-instance.e2e.test.ts` + worker `test/processes/concurrent-submitter.ts` — §28). Restam do §13: crash pós-commit/pré-ack e restart com consistência final. |
