...
| 7 — Test matrix | ✅ passo 0 (schema): `uq_wager_single_reversal` (migration + snapshot + enforcement — §27). ✅ **multi-instância ≥3 processos** (§28). ✅ **crash pós-commit/pré-ack + restart com consistência final** (`test/crash-recovery.e2e.test.ts` — §29). Resta só o diferencial opcional `bun run test:load`. |
